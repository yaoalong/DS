...
public class M2mQuorumCnxManager {
    private static final Logger LOG = LoggerFactory
            .getLogger(M2mQuorumCnxManager.class);
    ...
    public M2mQuorumCnxManager(M2mQuorumPeer self) {
        ...
        try {
            ...
            if (sid < 0) { // this is not a server id but a protocol version
                ...
                if (num_read != num_remaining_bytes) {
                    LOG.error("Read only " + num_read + " bytes out of "
    ...
    synchronized void connectOne(long sid) {
        if (senderWorkerMap.get(sid) == null) {
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Connected to server " + sid + "port:"
    ...
    /**
     * Flag that it is time to wrap up all activities and interrupt the
     * listener.
     */
    public void halt() {
        shutdown = true;
     ...
     */
    public M2mQuorumPeer getQuorumPeer() {
        return self;
    }
    ...
    class SendWorker extends Thread {
        ...
        /**
         * An instance of this thread receives messages to send through a queue
         * and sends them to the server sid.
         * 
         * @param sock
        ...
        public void run() {
            ...
            try {
                /**
                 * If there is nothing in the queue to send, then we send the
                 * lastMessage to ensure that the last message was received by
                 * the peer. The message could be dropped in case self or the
                 * peer shutdown their connection (and exit the thread) prior to
                 * reading/processing the last message. Duplicate messages are
                 * handled correctly by the peer.
                 *
                 * If the send queue is non-empty, then we have a recent message
                 * than that stored in lastMessage. To avoid sending stale
                 * message, we should send the message in the send queue.
                 */
                ArrayBlockingQueue<ByteBuffer> bq = queueSendMap.get(sid);
                if (bq == null || isSendQueueEmpty(bq)) {
                    ByteBuffer b = lastMessageSent.get(sid);
                    if (b != null) {
                        LOG.debug("Attempting to send lastMessage to sid="
                                + sid);
                        send(b);
                    }
                }
     ...
     * method removes an element from the head of the Queue and then inserts the
     * element at the tail. It can happen that the an element is removed by
     * another thread in {@link SendWorker#processMessage() processMessage}
    ...
    /**
     * Retrieves and removes buffer at the head of this queue, waiting up to the
     ...
     * to insert by the second thread to fail. Note that synchronizing this
     * method does not block another thread from polling the queue since that
     * synchronization is provided by the queue itself.
     *
     * @param msg
    ...
    /**
     * Retrieves and removes a message at the head of this queue, waiting up to
     * the specified wait time if necessary for an element to become available.
     *
     * {@link ArrayBlockingQueue#poll(long, TimeUnit)}
     */
