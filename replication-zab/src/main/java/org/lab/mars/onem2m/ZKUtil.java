...
public class ZKUtil {
     ...
     * <p>
     * If there is an error with deleting one of the sub-nodes in the tree, this
     * operation would abort and would be the responsibility of the app to
     * handle the same.
     *
     * 
     ...
     *            call back method
     * @param ctx
     *            the context the callback method is called with
     * @throws IllegalArgumentException
     *             if an invalid path is specified
     */
    public static void deleteRecursive(ZooKeeper zk, final String pathRoot,
            VoidCallback cb, Object ctx) throws InterruptedException,
            KeeperException {
        ...
        LOG.debug("Deleting " + tree.size() + " subnodes ");
        for (int i = tree.size() - 1; i >= 0; --i) {
            // Delete the leaves first and eventually get rid of the roo
        }
    }
     ...
     *            the zookeeper handle
     * @param pathRoot
     *            The znode path, for which the entire subtree needs to be
     *            listed.
